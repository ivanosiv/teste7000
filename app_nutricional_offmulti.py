...
GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY", "sua_chave_aqui")  # Substitua pela sua chave
...
def buscar_calorias(predicao: str) -> str | None:
    ...
    try:
        ...
        st.warning("Informação de calorias não disponível via Google.")
        return None

    except Exception:
        st.error("Não foi possível buscar a informação de calorias via Google.")
        return None

# ——— Função para obter todos os nutrientes de um arquivo Excel offline ———
def obter_todos_nutrientes(item: str) -> dict | None:
    """
    Lê o arquivo 'frutas_vegetais_nutrientes_completo.xlsx' e retorna o dicionário
    da linha correspondente ao 'item' (nome da fruta/vegetal). Caso não encontre,
    retorna None.
    """
    try:
        # Ajuste o nome do arquivo conforme necessário
        df = pd.read_excel('frutas_vegetais_nutrientes_completo.xlsx')

        # Presumindo que exista uma coluna chamada 'Item' ou 'item'
        coluna_item = None
        for col in df.columns:
            if col.strip().lower() == "item":
                coluna_item = col
                break

        if coluna_item is None:
            return None

        # Filtra por igualdade exata (case-insensitive)
        linha = df[df[coluna_item].str.lower() == item.lower()]
        if not linha.empty:
            return linha.iloc[0].to_dict()

    except FileNotFoundError:
        st.error("Arquivo Excel de nutrientes não encontrado.")
    except Exception:
        st.error("Erro ao ler o arquivo Excel de nutrientes.")
    return None

# ——— Função para preparar a imagem (PIL) para previsão ———
def preparar_imagem_pil(img_pil: Image.Image) -> str:
    """
    Recebe um objeto PIL.Image, redimensiona para (224,224), normaliza e faz a predição
    retornando o nome (label) capitalizado.
    """
...
# ——— Função principal para Classificação Offline de Múltiplos Itens ———
def run_offline_classifier():
    st.subheader("Classificação Offline (Multiplos Frutas/Vegetais)")

    img_file = st.file_uploader(
        "Carregar imagem contendo várias frutas/vegetais",
        type=["jpg", "png", "jpeg"]
    )
    if not img_file:
        return

    # 1) Exibe imagem original redimensionada apenas para visual
    img = Image.open(img_file).convert("RGB")
    st.image(img, caption="Imagem selecionada", use_container_width =False)

    # 2) Divide a imagem em 4 quadrantes e classifica cada um
    ...
    for idx, box in enumerate(quadrantes):
        crop = img.crop(box)
        # Verifica se o quadrante não está quase vazio (predominantemente branco)
        np_crop = np.array(crop.convert("L"))
        if np.mean(np_crop) > 245:  # Se for quase todo branco, considera sem fruta
            continue

        label = preparar_imagem_pil(crop)
        resultados.append(label)
        areas_com_imagem.append((idx, crop, label))

    if not resultados:
        st.warning("Não foi possível identificar nenhum objeto claro na imagem.")
        return

    # 3) Remove duplicados e exibe todos os resultados encontrados
    encontrados_unicos = sorted(set(resultados))
    st.info(f"**Itens detectados**: {', '.join(encontrados_unicos)}")

    # 4) Para cada item detectado, faz scraping de calorias → se falhar, usa Excel
    for item in encontrados_unicos:
        cal_text = buscar_calorias(item)
        if not cal_text:
            # Fallback Excel para calorias: 
            nutri = obter_todos_nutrientes(item)
            if nutri and 'Calorias (100g)' in nutri:
                cal_text = f"{nutri['Calorias (100g)']} kcal"
        if cal_text:
            st.markdown(
                f"""
                <div style="
                ...
                ">
                    <strong style="font-size:14px;">{item}: {cal_text} (por 100g)</strong>
                </div>
                """,
                unsafe_allow_html=True
            )
        else:
            st.info(f"Calorias para **{item}** não encontradas.")

    # 5) Exibe o perfil nutricional completo para cada item em forma de tabela
    for item in encontrados_unicos:
        todas_info = obter_todos_nutrientes(item)
        if todas_info:
            st.markdown(f"### Perfil Nutricional de {item.capitalize()} (por 100g)")
            dados_para_tabela = []
            for chave, valor in todas_info.items():
                if chave.strip().lower() == "item":
                    continue
                if pd.notna(valor):
                    dados_para_tabela.append({
                        "Nutriente": chave,
                        "Valor (por 100g)": valor
                    })
            df_exibir = pd.DataFrame(dados_para_tabela)
            st.table(df_exibir.set_index("Nutriente"))
        else:
            st.info(f"Nenhum dado nutricional completo disponível para **{item}**.")
...
def main():
    ...
    option = st.sidebar.selectbox(
        ...
        [
            ...
            "Classificação Offline (Múltiplos Itens)"
        ]
    )
    ...
    # Se escolher Classificação Offline (Frutas/Vegetais)
    if option == "Classificação Offline (Frutas/Vegetais)":
        run_offline_classifier()
        return

    # Se escolher Classificação Offline (Múltiplos Itens)
    if option == "Classificação Offline (Múltiplos Itens)":
        run_offline_classifier()
        return
