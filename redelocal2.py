...
def buscar_calorias(predicao: str) -> str | None:
    ...
    try:
        ...
        # 3) Varrer TODO o texto procurando algo como "123 kcal" ou "123 calorias"
...
# ——— Obtém IP local da máquina para expor o iframe corretamente ———
def get_local_ip() -> str:
    """
    Tenta descobrir o IP local da máquina na rede (não retorna 127.0.0.1).
    """
    try:
        # Cria um socket temporário para determinar o IP que se vê externamente
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        # Não precisa de um destino válido para conectar, basta um endereço público qualquer
...
# ——— Interface HTML do chat realtime via iframe (com IP dinâmico) ———
def realtime_chat_interface():
    local_ip = get_local_ip()
    chat_url = f"http://{local_ip}:3000"
    st.info(f"Servidor de chat realtime em: {chat_url}")
